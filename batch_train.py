    ...
    while iteration < opt.iterations + 1:
        ...
        for batch_data in training_dataloader:
            ...
            for batch_idx in range(batch_size):
                ...
                # save image and gt_image, they are torch.tensor 
                # 3 H W save them with cv2                
                Lssim = 1.0 - ssim(image, gt_image)
                loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * Lssim
                # do this every 100 iterations, from n*100 to n*100+11
                # counter = iteration % 100
                # if counter <= 11:
                #     save_dir = "/home/zhanpeng/code/SLAM/reconstruct/Instant4dGS/visualization/colmap"
                #     image_save = image.detach().cpu().numpy()
                #     gt_image_save = gt_image.detach().cpu().numpy()

                    
                #     image_save = image_save.transpose(1, 2, 0)*255
                #     gt_image_save = gt_image_save.transpose(1, 2, 0)*255
                    
                #     # change rgb dimension to bgr
                #     image_save = image_save[..., ::-1]
                #     gt_image_save = gt_image_save[..., ::-1]
                    
                #     cv2.imwrite(f"{save_dir}/image_iter_{iteration}.png", image_save)
                #     cv2.imwrite(f"{save_dir}/gt_image_iter_{iteration}.png", gt_image_save)
...
if __name__ == "__main__":
    ...
    network_gui_websocket.init("127.0.0.1", 6119)

    args = parser.parse_args(sys.argv[1:])
    cfg_dir = "/home/zhanpeng/code/SLAM/reconstruct/Instant4dGS/configs/sora"
    source_dir = "/home/zhanpeng/code/SLAM/reconstruct/MegaSAM_GS/voxel_filter/output/sora"
    model_dir = "/home/zhanpeng/code/SLAM/reconstruct/Instant4D/Densification/Instant4dGS/output/sora_vis_7"
    cfg_list = os.listdir(cfg_dir)
    for cfg_name in cfg_list:

        # already_run_cfg = ["doctor.yaml","batcat.yaml","sheep.yaml","coffee.yaml","pizza.yaml","panda.yaml", "einstein.yaml"]
        # already_run_cfg = ["doctor.yaml","batcat.yaml", "coffee.yaml", "pizza.yaml","panda.yaml", "einstein.yaml", "market.yaml"]
        already_run_cfg = ["doctor.yaml","batcat.yaml", "coffee.yaml"]
        if cfg_name in already_run_cfg:
            print(f"Already run {cfg_name}, skip")
            continue
        args.config = os.path.join(cfg_dir, cfg_name)
        ...
        op_ = op.extract(args)
        pp_ = pp.extract(args)
        lp_.source_path = os.path.join(source_dir, cfg_name.split(".")[0])
        lp_.model_path  = os.path.join(model_dir, "1",cfg_name.split(".")[0])
        lp_.sh_degree   = 0
        lp_.force_sh_3d = False
        
        op_.iterations = 5000
        op_.position_lr_max_steps= 50_00
        op_.densify_grad_threshold = 0.0002 /2 
        op_.densify_grad_t_threshold = 0.0002 / 80
