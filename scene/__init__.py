...
class Scene:
    ...
    def render_test_nvidia(self,path, gaussians, pipe, background):
        os.makedirs(os.path.join(path, "test"), exist_ok=True)
        
        cam_0 = next(cam for cam in self.train_cameras[1.0] if cam.timestamp == 0.0)
        cam_0 = cam_0.cuda()
        
        print(f"------cam_0.timestamp: {cam_0.timestamp}")
        og_timestamp = cam_0.timestamp
        time_total = len(self.train_cameras[1.0])
        print(f"------time_total: {time_total}")
        assert time_total == 12
        from gaussian_renderer import render
        for idx in range(time_total):
            timestamp = idx/(time_total-1)
            cam_0.timestamp = timestamp
            render_pkg = render(cam_0, gaussians, pipe, background)
            image, viewspace_point_tensor, visibility_filter, radii = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]

            image = image.cpu().numpy()
            image = image.transpose(1, 2, 0)
            image = (image * 255).astype(np.uint8)
            image_path = os.path.join(path, "test",  f"v000_t{idx:03d}.png")
            image = Image.fromarray(image)
            image.save(image_path)
        
        cam_0.timestamp = og_timestamp

    def render_evaluate_nvidia(self, path, gaussians, pipe, background):
        os.makedirs(os.path.join(path, "test"), exist_ok=True)
        
        cam_0 = next(cam for cam in self.test_cameras[1.0] if cam.timestamp == 0.0)
        cam_0 = cam_0.cuda()
        
        print(f"------cam_0.timestamp: {cam_0.timestamp}")
        og_timestamp = cam_0.timestamp
        time_total = len(self.test_cameras[1.0])
        print(f"------time_total: {time_total}")
        ...
        assert time_total == 12
        from gaussian_renderer import render
        for idx,cam in enumerate(self.test_cameras[1.0]):
            gt = cam.image.cuda()
            cam_0.timestamp = cam.timestamp
            
            t = int(cam.timestamp * (len(self.test_cameras[1.0])-1))

            render_pkg = render(cam_0, gaussians, pipe, background)
            image, viewspace_point_tensor, visibility_filter, radii = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]
            
            psnr_test += psnr(image, gt).mean().double()
            ssim_test += ssim(image, gt).mean().double()

                    
            image = image.cpu().numpy()
            image = image.transpose(1, 2, 0)
            image = (image * 255).astype(np.uint8)
            image_path = os.path.join(path, "test",  f"v000_t{t:03d}.png")
            image = Image.fromarray(image)
            image.save(image_path)
        
        print(f"------psnr: {psnr_test/time_total}")
        print(f"------ssim: {ssim_test/time_total}")
        

        
        self.evaluation_metrics["psnr"].append((psnr_test/time_total).item())
        self.evaluation_metrics["ssim"].append((ssim_test/time_total).item())
        
        cam_0.timestamp = og_timestamp
            
    def render_train(self, path, gaussians, pipe, background):
        os.makedirs(os.path.join(path, "train"), exist_ok=True)
        
        from gaussian_renderer import render
        for idx, cam in enumerate(self.train_cameras[1.0]):
            t = int(cam.timestamp * (len(self.train_cameras[1.0])-1))
            cam = cam.cuda()
            ...
            image = image.transpose(1, 2, 0)
            image = (image * 255).astype(np.uint8)
            image_path = os.path.join(path, "train",  f"v{t:03d}_t{t:03d}.png")
            image = Image.fromarray(image)
            image.save(image_path)
        
    def render_evaluate_dycheck(self, path, gaussians, pipe, background):
            os.makedirs(os.path.join(path, "test"), exist_ok=True)
            ...
            for idx,cam in enumerate(self.test_cameras[1.0]):
                ...
                test_id = round(cam.timestamp * (len(self.test_cameras[1.0])-1) / 3)
                ...
                image = (image * 255).astype(np.uint8)
                image_path = os.path.join(path, "test",  f"{test_id:05d}.png")
                image = Image.fromarray(image)
                image.save(image_path)
            
            print(f"------psnr: {psnr_test/len(self.test_cameras[1.0])}")
            print(f"------ssim: {ssim_test/len(self.test_cameras[1.0])}")
            
            
            self.evaluation_metrics["psnr"].append((psnr_test/len(self.test_cameras[1.0])).item())
            self.evaluation_metrics["ssim"].append((ssim_test/len(self.test_cameras[1.0])).item())
