...
def convert_nvidia_gs(cam_extrinsics, cam_intrinsics, images_folder):
    cam_infos = []
    # cam_extrinsics is a np.array in the format of (N_images, 3, 4)
    ...
    for idx in range(len(cam_extrinsics)):
        ...
        # we do not have to scale even the resolution change 
        # since we only need to keep short_side and focal's ratio
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                              depth=None, timestamp=idx/(len(cam_extrinsics)-1),
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        # scale the intrinsic considering the image size
        scale_x =  width / intr.width
        scale_y =  height / intr.height
        intr.params[0] = intr.params[0] * scale_x
        intr.params[1] = intr.params[1] * scale_y
        ...
        if intr.model=="SIMPLE_PINHOLE":
            ...
            assert False, "SIMPLE_RADIAL camera model not handled!"
            focal_length_x = intr.params[0]
...
def read_nvidia_poses_bounds(path):
    poses_bounds = np.load(
        os.path.join(path, "poses_bounds.npy")
    )  
    ...
    cam_inrin = poses[0, :, -1]  
    # H W focal
    # down right back to right back up llff convention
    ...
    return poses, cam_inrin



def readColmapSceneInfo(path, images, eval, llffhold=8, num_pts_ratio=1.0):
    
    cam_extrinsics, cam_intrinsics = read_nvidia_poses_bounds(path)
    # [n,3,4] [H,W,focal]

    reading_dir = "images" if images == None else images
    cam_infos_unsorted = convert_nvidia_gs(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(path, reading_dir))
...
def readCamerasFromCutter(path, transformsfile, white_background, extension=".png", time_duration=None, frame_ratio=1, dataloader=False):
    cam_infos = []
    ...
    def frame_read_fn(idx_frame):
        ...
        if time_duration is not None and 'time' in frame:
            if timestamp < time_duration[0] or timestamp > time_duration[1]:
                raise ValueError(f"Timestamp {timestamp} is out of range {time_duration}")
                return
        ...
        assert 'fl_x' in contents and 'fl_y' in contents and 'cx' in contents and 'cy' in contents

        fl_x = contents['fl_x']
        fl_y = contents['fl_y']
        cx = contents['cx']
        cy = contents['cy']
        FovY = focal2fov(fl_x, height)
        FovX = focal2fov(fl_y, width)
        # FovX = FovY = -1.0
...
def readCuterSceneInfo(path, white_background, eval, extension=".png", num_pts=100_000, time_duration=None, num_extra_pts=0, frame_ratio=1, dataloader=False):
    print("Reading Training Transforms")
    train_cam_infos = readCamerasFromCutter(path, "transforms_train.json", white_background, extension, time_duration=time_duration, frame_ratio=frame_ratio, dataloader=dataloader)
    print("Reading Test Transforms")
    test_cam_infos =  readCamerasFromCutter(path, "transforms_test.json" , white_background, extension, time_duration=time_duration, frame_ratio=frame_ratio, dataloader=dataloader)
    ...
    try:
        ...
        raise ValueError(f"Could not find {ply_path}")

    xyz = pcd.points
    colors = pcd.colors
    pcd = BasicPointCloud(points=xyz, colors=colors, normals=xyz)
...
sceneLoadTypeCallbacks = {
    "nvidia-colmap": readColmapSceneInfo,
    "Blender" : readNerfSyntheticInfo,
    "Cuter" : readCuterSceneInfo
}