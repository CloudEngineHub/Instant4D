...
def voxel_filter(droid_path, motion_path, save_dir, scene, use_mask=False):    
    ...
    scale_time_dynamic = np.repeat(3/((B-1)*10), pcd_dynamic.xyz.shape[0])
...
if __name__ == "__main__":
    ...
    for scene in scene_list:
        if use_msam:
            ...
            save_path = f"{save_dir}/{scene}"
            os.makedirs(save_path, exist_ok=True)
            Do_not_filter(droid_path, motion_path, save_path, scene, use_mask=True)
